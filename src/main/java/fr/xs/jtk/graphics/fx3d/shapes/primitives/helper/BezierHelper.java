 ...
 */
package fr.xs.jtk.graphics.fx3d.shapes.primitives.helper;
...
import fr.xs.jtk.graphics.fx3d.utils.GaussianQuadrature;
...
public class BezierHelper {
    ...
    public void preProcess(){
        ab=(Point3D) points.get(1).substract(points.get(0));
        bc=(Point3D) points.get(2).substract(points.get(1));
        cd=(Point3D) points.get(3).substract(points.get(2));
        
        abc=(Point3D) bc.substract(ab);
        bcd=(Point3D) cd.substract(bc);
        
        abcd=(Point3D) bcd.substract(abc);
    ...
    */
    private Point3D[] getTrihedron(double t){
        ...
        // r[t]
        Point3D R=(Point3D) points.get(0).multiply((float)Math.pow(1d-t, 3))
        ...
        // r'[t]
        Point3D dR=(Point3D) ab.multiply((float)(3d*Math.pow(1d-t, 2)))
                    .add(bc.multiply((float)(6d*(1d-t)*t))
                        .add(cd.multiply((float)(3d*Math.pow(t, 2))))); 
        float nT=dR.magnitude(); // || r'[t] ||
        
        // r''[t]
        Point3D ddR=(Point3D) abc.multiply((float)(6d*(1d-t))).add(bcd.multiply((float)(6d*t))); 
        // (|| r'[t] ||^2)'[t]
        ...
        Point3D dT=(Point3D) ddR.multiply(1f/nT).substract(dR.multiply(dn/((float)(Math.pow(nT,3d)*2d))));
        
        // T[t]=r'[t]/||r'[t]||
        Point3D T=(Point3D) dR.normalize();
        // N[t]=T'[t]/||T'[t]||
        Point3D N=(Point3D) dT.normalize();
        // B[t]=T[t]xN[t]/||T[t]xN[t]||
        Point3D B= new Point3D(T.crossProduct(N).normalize());
    ...
    public Point3D getS(int t, float cu, float su){
        ...
        // S[t,u]
        Point3D p = (Point3D) trihedron[BezierHelper.R]
