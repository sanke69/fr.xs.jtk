...
import fr.xs.jtk.graphics.fx3d.utils.FloatCollector;
import fr.xs.jtk.graphics.fx3d.utils.NormalMap;
import fr.xs.jtk.graphics.fx3d.utils.Palette;
import fr.xs.jtk.graphics.fx3d.utils.Patterns;
    ...
    */
    public int[] updateFacesWithIntersections(Point3D origin, Point3D direction,List<Point3D> points, List<Face3> faces){
        return faces.parallelStream().map(f->{
                ...
                Point3D edge1 = (Point3D) b.substract(a);
                Point3D edge2 = (Point3D) c.substract(a);
                Point3D pvec= new Point3D(direction.crossProduct(edge2));
                float det=edge1.dotProduct(pvec);
                
                int t0=0;
                if(det<=-EPS || det>=EPS){
                    float inv_det=1f/det;
                    Point3D tvec=(Point3D) origin.substract(a);
                    float u = tvec.dotProduct(pvec)*inv_det;
                    if(u>=0f && u<=1f){
                        Point3D qvec=(Point3D) tvec.crossProduct(edge1);
