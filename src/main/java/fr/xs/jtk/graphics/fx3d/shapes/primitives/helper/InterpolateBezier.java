    ...
    private void calculateControlPoints(){
        ...
        if(isClosed){
            ...
            for(int j=0; j<numSplines; j++){
                p1.add((Point3D) knots.get(j+1).substract(j==0?knots.get(numSplines-1):knots.get(j-1)).multiply(1/3f).add(knots.get(j)));
            }
            
            for(int j=0; j<numSplines; j++){
                Point3D p2=j==numSplines-1?(Point3D) knots.get(0).multiply(2f).substract(p1.get(0)):
                	(Point3D) knots.get(j+1).multiply(2f).substract(p1.get(j+1));
